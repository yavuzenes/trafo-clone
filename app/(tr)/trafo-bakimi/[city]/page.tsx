...
export default async function Page({ params }: Props) {
  ...
  return <><Header/><main>
    <section className="page-hero city-hero"><div className="shell"><Breadcrumb items={[{ label: "Trafo Bakım ve Onarım", href: "/hizmetler/trafo-bakim-ve-onarimi" }, { label: item.name }]}/><span className="kicker">ANKARA MERKEZLİ · SAHA PLANI İLE TEKNİK HİZMET</span><h1>{item.name} trafo bakımı ve onarımı</h1><p>{item.name} bölgesindeki {item.context} için bakım, arıza tespiti ve elektriksel test kapsamını tesisin ekipman durumuna göre planlıyoruz.</p><div className="actions"><Link className="btn primary" href="/iletisim">Teknik talep oluşturun</Link><Link className="btn dark-outline" href="/hizmetler/trafo-bakim-ve-onarimi">Hizmet kapsamını inceleyin</Link></div></div></section>
    <section className="city-work-note"><div className="shell"><strong>{item.name} saha deneyimi</strong><p>BES Enerji ekibi {item.name} ilinde trafo ve OG sistemleri için kurulum, arıza tespiti, onarım ve test çalışmaları gerçekleştirmiştir. Yeni bir talepte uygulanacak kapsam, tesisin ekipman bilgileri ve güvenlik koşullarına göre belirlenir.</p></div></section>
    ...
    <section className="section"><div className="shell city-incident"><div><span className="kicker">ÖLÇÜM VE RAPORLAMA</span><h2>Bakım kararını hangi veriler destekler?</h2></div><div><p>Çevirme oranı, sargı direnci ve izolasyon ölçümleri farklı arıza mekanizmalarını araştırır. Test seçimi trafonun gücü, tipi, yaşına ve üretici dokümantasyonuna göre yapılır; her test her ekipmana otomatik olarak uygulanmaz.</p><p>Ölçüm sonuçları; etiket bilgileri, ortam koşulları, önceki raporlar ve varsa koruma olay kayıtlarıyla karşılaştırılır. Bulgular bakım, onarım veya ileri inceleme önerisiyle birlikte teknik rapora aktarılır.</p><Link href="/cihaz-parkuru">Kullanılan test cihazlarının özelliklerini inceleyin →</Link></div></div></section>
    ...
    <section className="section city-links"><div className="shell"><span className="kicker">İL BAZINDA TEKNİK KAPSAM</span><h2>Diğer iller için trafo bakım bilgileri</h2><div className="city-link-grid">{cityPages.filter((entry) => entry.slug !== item.slug).map((entry) => <Link key={entry.slug} href={`/trafo-bakimi/${entry.slug}`}>{entry.name} trafo bakımı</Link>)}</div></div></section>
