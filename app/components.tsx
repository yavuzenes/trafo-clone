...
export function Footer() {
  return <><ReferenceStrip/><section className="bes-cta-band"><div className="shell bes-cta-inner"><div><span className="kicker">TEKNİK DESTEK VE KEŞİF</span><h2>Enerji sürekliliğinizi birlikte planlayalım.</h2><p>Trafo ve OG hücre sistemleriniz için bakım, arıza veya test talebinizi iletin.</p></div><div><a className="bes-button bes-button-light" href={`tel:${company.phone}`}>Hemen arayın</a><Link className="bes-button bes-button-outline-light" href="/iletisim">Teklif isteyin</Link></div></div></section><footer className="bes-footer"><div className="shell bes-footer-grid"><div><Link href="/" className="bes-footer-brand"><Image src="/images/bes-enerji-2026-logo.jpg" alt="BES Enerji" width={220} height={87}/></Link><p>Trafo ve OG hücre sistemlerinde arıza tespiti, bakım, onarım ve elektriksel test odaklı teknik servis.</p></div><div><h3>Hizmetlerimiz</h3>{coreServices.slice(0,6).map(service=><Link key={service.slug} href={`/hizmetler/${service.slug}`}>{service.shortTitle}</Link>)}</div><div><h3>Kurumsal</h3><Link href="/kurumsal">Hakkımızda</Link><Link href="/faaliyet-alanlari">Faaliyet Alanları</Link><Link href="/cihaz-parkuru">Cihaz Parkuru</Link><Link href="/yetkili-servisler">Yetkili Servisler</Link><Link href="/partnerler">Partnerler</Link><Link href="/referanslar">Referanslar</Link><Link href="/sss">Sık Sorulan Sorular</Link></div><div><h3>İletişim</h3><a href={`tel:${company.phone}`}>{company.phoneDisplay}</a><p>{company.address}</p><Link href="/iletisim">İletişim bilgileri →</Link></div></div><div className="shell bes-copyright">© 2026 BES Enerji. Tüm hakları saklıdır.</div></footer><div className="sticky-actions"><a href={`tel:${company.phone}`}>Ara</a><a href="https://wa.me/905532124761" target="_blank" rel="noreferrer">WhatsApp</a></div></>;
}
